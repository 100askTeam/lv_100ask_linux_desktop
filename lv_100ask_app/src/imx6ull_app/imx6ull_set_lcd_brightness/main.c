...
#include "src/set_lcd_brightness.h"
...
int main(void)
{
    ...
    // 调用进程间通信管理初始化函数
	lv_100ask_dbus_handler_init("net.ask100.lvgl.SetBrightness", "/net/ask100/lvgl/SetBrightness");

    // 让桌面暂停运行
    dbus_method_call("net.ask100.lvgl.Main", "/net/ask100/lvgl/Main", "net.ask100.lvgl.Main", "states", 0, 0);

    /*Create a Demo*/
    imx6ull_set_lcd_brightness_init();
